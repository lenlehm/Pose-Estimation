...
peak_thresh = 2;
[f1,d1] = vl_sift(single(image1),'PeakThresh', peak_thresh);

P2 = fullfile("tracking\",sprintf('DSC_9%d.JPG',n+1));
image2 = imread(P2);
image2 = rgb2gray(image2);
peak_thresh = 2;
[f2,d2] = vl_sift(single(image2),'PeakThresh', peak_thresh);

[matches, scores] = vl_ubcmatch(d1, d2, 3.6);

matchesPoints1 = transpose(f1(1:2, matches(2, 200:300)));
matchesPoints2 = transpose(f2(1:2, matches(2, 200:300)));

figure; ax = axes;
showMatchedFeatures(image1,image2, matchesPoints1, matchesPoints1,'montage','Parent',ax);
...
% %image_corners_estimate = image_point(1:8, :);
...
%     %Estimate Jacobian
%     e = 0.000001;
%     J(:, 1) = ( projectOntoImage(x+[e; 0; 0; 0; 0; 0], transpose(result), A) - y0) / e;
%     J(:, 2) = ( projectOntoImage(x+[0; e; 0; 0; 0; 0], transpose(result), A) - y0) / e;
%     J(:, 3) = ( projectOntoImage(x+[0; 0; e; 0; 0; 0], transpose(result), A) - y0) / e;
%     J(:, 4) = ( projectOntoImage(x+[0; 0; 0; e; 0; 0], transpose(result), A) - y0) / e;
%     J(:, 5) = ( projectOntoImage(x+[0; 0; 0; 0; e; 0], transpose(result), A) - y0) / e;
%     J(:, 6) = ( projectOntoImage(x+[0; 0; 0; 0; 0; e], transpose(result), A) - y0) / e;
%     
%     dy = y0 - y;
% 
