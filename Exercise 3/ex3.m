...
% image = imread('images/tracking/DSC_9775.JPG');            
...
image_pre = imread(sprintf('images/tracking/DSC_9%d.JPG',j));
...
image_curr = imread(sprintf('images/tracking/DSC_9%d.JPG',j+1));
...
for i = 1 : N
    ...
    if i == 1 
        fprintf('initail err %d\n' , norm(e1));
    end
    err = [e1(1:size(M,1)) e1(size(M,1)+1 :2*size(M,1))];
    err = err(:,1) + err(:,2);
    alpha =  1.48257968 * median(abs(e1))  ;
...
%     e = 0.00001;
%     J = zeros(size(e1,1),6);
%     J(:, 1) = ( projection_err(cameraParams,theta+[e 0 0 0 0 0],M,m)) / e;
%     J(:, 2) = ( projection_err(cameraParams,theta+[0 e 0 0 0 0],M,m)) / e;
%     J(:, 3) = ( projection_err(cameraParams,theta+[0 0 e 0 0 0],M,m)) / e;
%     J(:, 4) = ( projection_err(cameraParams,theta+[0 0 0 e 0 0],M,m)) / e;
%     J(:, 5) = ( projection_err(cameraParams,theta+[0 0 0 0 e 0],M,m)) / e;
%     J(:, 6) = ( projection_err(cameraParams,theta+[0 0 0 0 0 e],M,m)) / e;
    J  = Jacob( theta,M,m );
%     delta = -inv(J'* J + lamda * eye(6)) * (J' * e1) ;
    delta = inv(J'* W* J + lamda * eye(6)) * (J' *W * e1) ;
    e_new = projection_err(cameraParams,theta + delta' ,M,m) ;
    if norm(e_new) > norm(e1) 
        lamda = 10 * lamda ;
%         disp('gradient');
    else 
        lamda = 0.1 * lamda ;
