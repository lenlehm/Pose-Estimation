% Load the given camera intrinsic Matrix
intrinst_m = [2960.37845 0 0;
                0  2960.37845 0;
                1841.68855 1235.23369 1];
cameraParams = cameraParameters('IntrinsicMatrix',intrinst_m);
% read the image       
image = imread('tracking/DSC_9775.JPG');    
% get pose estimation via PnP and RANSAC
...
theta = [rotationVector translationVector]; %(thetaX, thetaY, thetaZ, t_x, t_y, t_z)
N = 100 ;
lamda = 0.001 ; % according to exercise Sheet
c = 4.685 ; 
% obtain matches of consecutive frame
M = result(matches(1,:),:) ; 
m = image_point_cur ;
...
%     W = diag(v);
    ...
    % calculate the jacobian to minimize error
    e = 0.000001;
    J = zeros(size(e1,1),6);
...
%     delta = -inv(J'* J + lamda * eye(6)) * (J' * e1) ;
    % Levenberg Marquardt - exercise sheet
...
% get the image point of the consecutive frame
point = worldToImage(cameraParams,rotationMatrix,theta(4:6),M);
% get the image points of the databse obejct
